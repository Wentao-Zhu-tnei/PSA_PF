...
class App(tk.Tk):
    def __init__(self):
        ...
        self.title('PSA Event Generator')
        ...
        self.LabelFrame20 = ttk.LabelFrame(frame6, text='Update Existing Events')
        # -----------------------------------------------------------
        '''Frame 0: Select Maint File'''
        self.frame0 = ttk.LabelFrame(self, text='Select Events File')
        ...
        # -----------------------------------------------------------
        '''Frame 001: Select Asset File'''
        self.frame001 = ttk.LabelFrame(self.LabelFrame00, text='Select Asset File')

        self.fe2 = ExcelEntry(self.frame001)
        ...
        self.description = tk.Label(frame1, text='Reason For Event')
        self.description.pack(padx=10, pady=5)

        self.description_entry = tk.Text(frame1, height=3)
        self.description_entry.insert('1.0', 'Planned event')
        ...
        # -----------------------------------------------------------
        '''Input amount to Dispatch in kW'''
        self.frame01 = tk.Frame(self.LabelFrame00)

        self.dispatch_label = tk.Label(self.frame01, text='kW to Dispatch')
        self.dispatch_label.grid(row=0, column=0, padx=5, pady=5)

        self.dispatch = tk.IntVar(self.frame01, 35)
        self.dispatch_entry = tk.Entry(self.frame01, textvariable=self.dispatch, font=('TkDefaultFont', 10), width=15)
        self.dispatch_entry.grid(row=1, column=0, padx=5, pady=5)
        ...
        self.update_service_btn = tk.Button(frame5, text='Update Events', command=self.update_service, bg='green', fg='white')
        ...
        self.frame01.grid(column=0, row=4, columnspan=2, padx=10, pady=0)
        ...
        self.add_service_btn.grid(pady=15, column=0, row=5, sticky=tk.E) 
        self.undo_btn = tk.Button(self.LabelFrame00, text='Undo', command=self.undo, bg='black', fg='white')
        self.undo_btn.grid(pady=15, column=1, row=5, sticky=tk.W)
        ...
        # -----------------------------------------------------------
        '''Final Packing'''
        self.LabelFrame20.grid(padx=10, pady=0, column=0, row=1, sticky=tk.NE) # Update Events
            ...
            if isinstance(self.asset_dict, dict) and self.fe1.dir_box.get("1.0", "end").strip() != const.strNoFileDisp:
                ...
                try:
                    ...
                    try:
                        ...
                        df = df[df['ASSET_TYPE'] == const.strAssetGen]
    ...
    def add_service(self):
        ...
        if '.xlsx' not in file:
            ...
            file = folder + f'{datetime.strftime(self.ct, "%d-%m-%y")} {const.strEventFile}_0.xlsx'
            i = 0
            ...
            df = pd.DataFrame(columns=['ASSET_LONG_NAME','ASSET_ID','ASSET_TYPE','START_SERVICE','END_SERVICE','DURATION','P_DISPATCH_KW','DESCRIPTION','SCENARIO'])
        ...
        for scenario in [self.add_b.get(), self.add_m.get(), self.add_c.get(), self.add_mc.get()]:
            if scenario != '0':
                df.loc[len(df)] = [asset_id, asset_id, asset_type, start_date, end_date, difference, dispatch, description, scenario]
        try:
            pa.auto_expand_and_save(df, file, index=False, sheet_name=const.strDataSheet)
            self.refresh_image()
            print(f'Saved to: {file}')
    ...
    def refresh_image(self, event=None):
        ...
        try:
            self.plot = Image.open(pa.plot_gantt_outages('', directory=file, no_of_days=days, save=True, out_type='EVENT')).resize((self.cx,self.cy))
